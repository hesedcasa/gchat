...
describe('config', () => {
  ...
  describe('addUser', () => {
    it('adds a user with a normalized user ID', async () => {
      ...
      const result = await addUser('/tmp/config', 'Jane Doe', '123456789012345678901')
    ...
    it('keeps a users/ prefixed ID as-is', async () => {
      ...
      await addUser('/tmp/config', 'Jane Doe', 'users/123')
    ...
    it('overwrites an existing user with the same name', async () => {
      ...
      await addUser('/tmp/config', 'Jane Doe', 'users/new')
